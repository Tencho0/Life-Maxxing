...
- [x] Body-weight module (see docs/superpowers/specs/2026-06-04-weight-tracking-design.md + plan docs/superpowers/plans/2026-06-04-weight-tracking.md). Extends the app beyond the functional spec. One entry per day stored as **integer grams** (`WeightLogs`, `UNIQUE(date)`, `CHECK(weight_grams > 0)`, edit-on-existing like Steps); `WeightDao` mirrors `StepsDao`; `WeightSummary` + `computeWeight` (latest/change/min/max/count). Schema **v2→v3** adds `weight_logs`. Wired into ZIP backup (round-trips), clear-all-data (`_deleteAll`), dev seed (30-day trend), and AI export (`ExportModule.weight`, JSON `weightLogs` integer grams, Markdown weight section). UI: a 5th **"Тегло"** tab in the Health screen (list + add/edit via `showWeightSheet`, one-per-day) and a vitals weight card (latest + signed Δ via `formatKg`/`formatKgDelta`, green trend `Sparkline`). Tests: summary (3), DAO (5), backup round-trip + clearAllData, export JSON+Markdown+module-scope, format helpers, form one-per-day create/edit, Health-screen tab render. **Device-pass owns final sign-off** (v2→v3 migration on a real device, APK build, chart visual).
